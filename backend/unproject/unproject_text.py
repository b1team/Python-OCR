...
import cv2
from backend.unproject import ellipse
...
def centered_warp(u0, v0, a, b):
    return np.dot(
        translation(u0, v0), np.dot(perspective_warp(a, b), translation(-u0, -v0))
    )
...
def threshold(img):
    ...
    return cv2.adaptiveThreshold(
        img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 101, 21
    )
...
def get_conics(img, contours, hierarchy, abs_area_cutoff=0.0001, mean_area_cutoff=0.15):
    ...
    display = img.copy()
    cv2.drawContours(display, okcontours + allchildren, -1, (0, 255, 0), 6, cv2.LINE_AA)
    ...
    for c, a in zip(okcontours, areas):
        ...
        cv2.putText(
            display, s, ctr, cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 0, 0), 12, cv2.LINE_AA
        )

        cv2.putText(
            display, s, ctr, cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 0), 6, cv2.LINE_AA
        )
    ...
    print(
        "got {} contours with {} small.".format(
    ...
    if 0:
        ...
        plt.plot([0, 3000], np.polyval(xfit, [0, 3000]), "b--", zorder=0, label=xlabel)
        plt.plot([0, 3000], np.polyval(yfit, [0, 3000]), "r--", zorder=0, label=ylabel)
...
def optimize_conics(conics, p0):
    ...
    if 0:
        ...
        for i, (rect, (x, H, fval)) in enumerate(zip(rects, opt_results)):
            ...
            cv2.putText(
                display,
                "Area discrepancy: {:.3f}".format(fval),
                (16, height - 24),
...
def orientation_detect(img, contours, H, rho=8.0, ntheta=512):
    ...
    for i, theta in enumerate(thetas):
        ...
        bin_idx = (
            (-(u - u0) * np.sin(theta) + (v - v0) * np.cos(theta))  # x term
            * irho  # y term, both
            # divided by pixels
            # per bin
            + 0.5 * bin_max
    ...
    if 1:  # just debug visualization
        ...
        warped = cv2.warpPerspective(
            img, TH, (shape[1], shape[0]), borderMode=cv2.BORDER_REPLICATE
        )
...
def un_project(img):
    ...
    un_projected_image = skew_detect(img, contours, RH)
    return un_projected_image

def main():

    img = cv2.imread(sys.argv[1])
    un_projected_image = un_project(img)
    cv2.imshow('Debug', un_projected_image)
